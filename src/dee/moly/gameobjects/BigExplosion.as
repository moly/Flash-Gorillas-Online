package dee.moly.gameobjects {
	...
	public class BigExplosion extends GameObject{
		...
		for (var i:int = 0; i < 2; ++i){
			if (stage == STAGE_1) {
				DrawingBitmap(canvas).circle(position.x + 4 * SCLX + XADJ, position.y + 7 * SCLY + YADJ, currentRadius, EXPLOSION_COLOUR1, 1.57);
			...
			if (stage == STAGE_2) {
				...
				if (currentRadius < 8 * SCLX)
					DrawingBitmap(canvas).circle(position.x + 3.5 * SCLX + XADJ, position.y + 7 * SCLY + YADJ, (8 * SCLX + 1) - currentRadius, 0xFF0000AD, 1.57);
				DrawingBitmap(canvas).circle(position.x + 3.5 * SCLX + XADJ, position.y + YADJ, currentRadius, currentRadius % 2 == 0 ? EXPLOSION_COLOUR1 : EXPLOSION_COLOUR2, 1.57);
			}
			if (stage == STAGE_3) {
				DrawingBitmap(canvas).circle(position.x + 3.5 * SCLX + XADJ, position.y + YADJ, currentRadius, 0xFF0000AD, 1.57);
				if (--currentRadius <= 0)
					stage = FINISHED;
			}
		}
